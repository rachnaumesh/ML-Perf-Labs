...
int main() {
    ...
    // Initialize weights once outside the loop to avoid repeated initialization
    GPT2Weights weights = initialize_weights();
    ...
    for (int i = 0; i < WARMUP_RUNS; i++) {
        ...
        // printf("Warmup run %d: Predicted token: %d\n", i + 1, max_index);
        free(logits);
    }
    ...
    for (int run = 0; run < NUM_RUNS; run++) {
        ...
        // printf("Run %3d: Time: %.4f seconds, Predicted token: %d\n", 
    ...
    // Print statistics
    printf("\nBenchmark Summary:");
    printf("Average time per run: %.4f seconds", total_time / NUM_RUNS);
    printf("Total time for %d runs: %.4f seconds", NUM_RUNS, total_time);
    ...
    for (int i = 1; i < NUM_RUNS; i++) {
        if (predicted_tokens[i] != predicted_tokens[0]) {
            printf("Warning: Inconsistent prediction at run %d: %d vs %d\n",
                   i + 1, predicted_tokens[i], predicted_tokens[0]);
            consistent = false;
        }
    }
    
    if (consistent) {
        printf("\nAll runs produced consistent token predictions: %d\n", 
               predicted_tokens[0]);
    }
