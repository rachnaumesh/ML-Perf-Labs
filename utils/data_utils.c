...
float ****init_kernel_multiple_channels(float kernel_data[][2][3][3], int numFilters, int kernelSize, int numChannels)
{
    float ****kernel = malloc(numFilters * sizeof(*kernel));
    for (int i = 0; i < numFilters; i++)
    {
        kernel[i] = malloc(numChannels * sizeof(*kernel[i]));
        for (int c = 0; c < numChannels; c++)
        {
            kernel[i][c] = malloc(kernelSize * sizeof(*kernel[i][c]));
            for (int j = 0; j < kernelSize; j++)
            {
                kernel[i][c][j] = malloc(kernelSize * sizeof(*kernel[i][c][j]));
                for (int k = 0; k < kernelSize; k++)
                {
                    kernel[i][c][j][k] = kernel_data[i][c][j][k];
                }
            }
        }
    }
