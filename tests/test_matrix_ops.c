...
void test_matmul_identity_matrix(void)
{
    float **A = (float **)malloc(2 * sizeof(float *));
    for (int i = 0; i < 2; i++)
    {
...
void test_matmul_large_matrices(void)
{
    // Setup
    float **A = (float **)malloc(100 * sizeof(float *));
    for (int i = 0; i < 100; i++)
    {
        A[i] = (float *)malloc(100 * sizeof(float));
    }
    for (int i = 0; i < 100; i++)
    {
        for (int j = 0; j < 100; j++)
    ...
    for (int i = 0; i < 100; i++)
    {
        B[i] = (float *)malloc(100 * sizeof(float));
    }
    for (int i = 0; i < 100; i++)
    {
        for (int j = 0; j < 100; j++)
    ...
    float **expected = (float **)malloc(100 * sizeof(float *));
    for (int i = 0; i < 100; i++)
    {
        expected[i] = (float *)malloc(100 * sizeof(float));
    }
    for (int i = 0; i < 100; i++)
    {
        for (int j = 0; j < 100; j++)
    ...
    // Run function under test
    float **C = matmul(A, B, 100, 100, 100, 100);

    // Check expectations
    assert_float_array_equal_matmul(expected, C, 100, 100);
