...
float ***convolution_im2col(float ***image, int numChannels, float ****kernel, float *biasData, int numFilters, int inputSize, int kernelSize, MatmulType matmul_type)
{
    ...
    else if (matmul_type == MATMUL_THREAD)
    {
        matmul_result = matmul_thread(flattened_kernels, im2col_result, numFilters, numChannels * kernelSize * kernelSize, numChannels * kernelSize * kernelSize, outputSize);
    }
